// resharper disable all - this isn't our code

// copied from https://github.com/timcassell/ProtoBenchmarkHelpers/blob/main/ProtoBenchmarkHelpers/BenchmarkThreadHelper.cs
// Licensed under the MIT License

// needed until https://github.com/dotnet/BenchmarkDotNet/issues/147 is resolved
...
namespace Proto.Utilities.Benchmark
{
    ...
    /// </summary>
    public class
        BenchmarkThreadHelper : IDisposable,
        IEnumerable // IEnumerable only included because the C# compiler requires it to use collection initializer.
    {
        ...
        ///     Sets the maximum number of threads that can run actions in parallel. The default of -1
        ///     will use <see cref="Environment.ProcessorCount" />.
        /// </param>
        public BenchmarkThreadHelper(int maxConcurrency = -1)
        {
            ...
            else if (maxConcurrency == 0 || maxConcurrency < -1)
            {
                throw new ArgumentOutOfRangeException(nameof(maxConcurrency),
                    "maxConcurrency must be positive or -1. Value: " + maxConcurrency);
            }
        ...
        public void Add(Action action)
        {
            ...
            if (runningThreadCount <
                maxConcurrency - 1) // Subtract 1 because we already have the caller thread to work with.
            {
        ...
        /// </summary>
        /// <remarks>
        ///     If any action throws an exception, this will throw an <see cref="AggregateException" /> containing all thrown
        ///     exceptions.
        /// </remarks>
